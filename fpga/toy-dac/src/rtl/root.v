...
// MODULATOR_USE_SYNTH=0 → legacy Pascal weights (auto-generated inside
// dac.v from MODULATOR_ORDER, ORDER∈{1,2}). =1 → load synthesised CIFB
// coefficients from dsm_coeffs.vh (regenerated by
// scripts/synthesize_dsm.py). The .vh fixes ORDER, OSR and Q-format,
// so MODULATOR_ORDER is ignored on that path.
parameter MODULATOR_USE_SYNTH = 1;
...
// Two paths, selected at elaboration time by MODULATOR_USE_SYNTH:
//   0 → Pascal default (dac.v auto-generates coefficients for
//       ORDER∈{1,2} when DSM_A is left at its zero default).
//   1 → synthesised CIFB coefficients pulled from dsm_coeffs.vh.
// Localparams from the .vh are scoped inside the genblock so the
// header can be re-included safely.
generate
if (MODULATOR_USE_SYNTH) begin : g_dac_synth
    ...
    dac #(
        ...
        .clk(mclk), .rst(rst),
        .din(dac_din_left), .dvalid(dac_dvalid_l),
    ...
    dac #(
        ...
        .clk(mclk), .rst(rst),
        .din(dac_din_right), .dvalid(dac_dvalid_r),
        .dither1(dither1), .dither2(dither2),
        .dout(dac_raw_r)
    );
end else begin : g_dac_pascal
    dac #(
        .WORDLENGTH(MODULATOR_WORDLENGTH),
