...
module frac_servo #(
    ...
    wire signed [31:0] integ_n =
        (integ_n_raw > STEP_ADJ_MAX) ? STEP_ADJ_MAX :
        (integ_n_raw < STEP_ADJ_MIN) ? STEP_ADJ_MIN :
        integ_n_raw[31:0];

    // ── Proportional term: KP · err (transient) ─────────────────
    wire signed [31:0] prop      = $signed(KP[15:0]) * err;
    wire signed [33:0] pi_n_raw  = integ_n + prop;
    wire signed [31:0] pi_n =
        (pi_n_raw > STEP_ADJ_MAX) ? STEP_ADJ_MAX :
        (pi_n_raw < STEP_ADJ_MIN) ? STEP_ADJ_MIN :
        pi_n_raw[31:0];
