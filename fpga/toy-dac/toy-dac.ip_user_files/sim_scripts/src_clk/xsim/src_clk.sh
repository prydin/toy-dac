...
#
# Script generated by Vivado on Sat Apr 25 08:09:07 -0400 2026
...
#
# Usage        : src_clk.sh
#                src_clk.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                src_clk.sh [-reset_run]
#                src_clk.sh [-reset_log]
#                src_clk.sh [-help]
#
...
# script info
echo -e "src_clk.sh - Script generated by export_simulation (Vivado v2024.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./src_clk.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xelab --incr --debug typical --relax --mt 2  -L xil_defaultlib -L unisims_ver -L unimacro_ver -L secureip -L xpm --snapshot src_clk xil_defaultlib.src_clk xil_defaultlib.glbl -log elaborate.log
}

# RUN_STEP: <simulate>
simulate()
{
  xsim src_clk -key {Behavioral:sim_1:Functional:src_clk} -tclbatch cmd.tcl -log simulate.log
}
...
reset_run()
{
  files_to_remove=(xelab.pb xsim.jou xvhdl.log xvlog.log compile.log elaborate.log simulate.log xelab.log xsim.log run.log xvhdl.pb xvlog.pb src_clk.wdb xsim.dir libdpi.so)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./src_clk.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: src_clk.sh [-help]\n\
Usage: src_clk.sh [-step]\n\
Usage: src_clk.sh [-lib_map_path]\n\
Usage: src_clk.sh [-reset_run]\n\
Usage: src_clk.sh [-reset_log]\n\
Usage: src_clk.sh [-keep_index]\n\
Usage: src_clk.sh [-noclean_files]\n\n\
